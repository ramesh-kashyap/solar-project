...
class Dashboard extends Controller
{
    ...
    public function add_wallet(Request $request)
    {
        ...
        try {
            $data = $request->all();
            $rules = array('upiId1' => 'required', 'upiId2' => 'required');
            $msg = [
                'upiId1.required'     => 'upiId1 is required',
                'upiId2.required'         => 'upiId3 is required' ,
            ];

            $validator = Validator::make($data, $rules, $msg);
            if ($validator->fails())
                return Redirect::back()->withErrors($validator->getMessageBag()->first());
                 $general= \DB::table('general_settings')->where('id',1)->first();

                 if ($request->hasFile('qrcode1')) {
                     $publicPath = public_path('uploads/'.$general->qrcode1);
               
                    if (File::exists($publicPath)) {
                        File::delete($publicPath);
                    }
                            
                     $qrcode1 = $request->file('qrcode1');
                    $imageName = time().'.'.$qrcode1->extension();
                    $request->qrcode1->move(public_path('uploads/'),$imageName);

                 }
                 else
                 {
                   $imageName =$general->qrcode1;
                 } 
                 
                 if ($request->hasFile('qrcode2')) {
                     $publicPath = public_path('uploads/'.$general->qrcode2);
               
                    if (File::exists($publicPath)) {
                        File::delete($publicPath);
                    }
                            
                     $qrcode2 = $request->file('qrcode2');
                    $imageName2 = time().'.'.$qrcode2->extension();
                    $request->qrcode2->move(public_path('uploads/'),$imageName2);

                 }
                 else
                 {
                   $imageName2 =$general->qrcode2;
                 }
               
           DB::table('general_settings')
            ->where('id', 1)
            ->update(['upiId1' => $data['upiId1'],'upiId2' => $data['upiId2'],'message'=>$data['message'],'usdt_address'=>$data['usdt_address'],'qrcode1'=>$imageName,'qrcode2'=>$imageName2]);
 ...
 public function addreturn()
    {     
     ...
     $kb= \DB::table('general_settings')->where('id',1)->first(); 
     $this->data['general'] = $kb;
...

{
    try{
      ...
      $bank_array['upi_id']=$post_array['upi_id'];
