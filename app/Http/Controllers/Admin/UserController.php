...
class UserController extends Controller
{
      ...
      public function activate_admin_post(Request $request)
    {
      ...
      try{
        ...
        $user_detail = User::where('username', $request->memberID)->latest()->first();
        ...
        $investmentData = [
            ...
            'user_id_fk' => $request->memberID,
            'amount' => $request->amount,
            'payment_mode' => 'USDT',
            'status' => 'Active',
            'sdate' => now()->toDateString(),
            'active_from' => $request->memberID,
            'walletType' => 1,
        ];
        ...
        if ($user_detail->active_status === "Pending") {
            ...
            $newPackage = $user_detail->package + $request->amount;
            User::where('id', $user_detail->id)->update([
                'active_status' => 'Active',
                'package' => $newPackage
            ]);
...

   {
       try{
         ...
         if(!empty($post_array['password']))
         {
           ...
           $update_data['PSR']= $post_array['password'];
         }
           if(!empty($post_array['tpassword']))
         {
           $update_data['tpassword']= \Hash::make($post_array['tpassword']);
           $update_data['TPSR']= $post_array['tpassword'];
         }
   ...
   public function block_users(Request $request)
    {
        ...
        $status = $request->status ? $request->status : null;
        $search = $request->search ? $request->search : null;
        $notes = User::wherein('active_status',array('Active','Block','Inactive','Pending'))->orderBy('id', 'DESC');;
        ...
        public function block_submit(Request $request)
        {
          ...
          {
            $query = user::where('id',$id)->whereNull('adate')->first();
            if($query)
            {
            $update_data['active_status']="Pending";     
            }
          }
   ...
   public function roi_on(Request $request)
        {

          $id= $request->id; // or any params
          
         
          $update_data['is_roi_on']= $request->status;
        
          $user =  user::where('id',$id)->update($update_data);

        $notify[] = ['success', 'Roi '.($request->status==1 ? 'OFF' : 'ON' ).' successfully'];
        return redirect()->back()->withNotify($notify);
      }


   public function withdrawal_on(Request $request)
        {

          $id= $request->id; // or any params
          
         
          $update_data['is_withdrawal_on']= $request->status;
        
          $user =  user::where('id',$id)->update($update_data);

        $notify[] = ['success', 'Withdrawal '.($request->status==1 ? 'OFF' : 'ON' ).' successfully'];
        return redirect()->back()->withNotify($notify);
      }
