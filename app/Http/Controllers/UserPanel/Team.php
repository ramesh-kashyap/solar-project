...
class Team extends Controller
{
    ...
    // public function tree($round){

    //      $user= Auth::user();
    //      $table = 'round'.$round;
    //     if($user){
    //         $this->data['tree'] = showTreePage($user->username,$table);
    //       $this->data['page'] = 'user.team.tree';
    ...
    public function tree($round)
{
    ...
    if ($user) {
        // ✅ Fetch distinct round values for the logged-in user
        $userRounds = Upgrade::where('round', $round)->where('user_id',$user->id)->first();
        ...
        // ✅ Send rounds to blade
        $this->data['userRounds'] = $userRounds;
...
public function genealogy2(Request $request)
{
   $tuser = $request->user_id_fk;
$suser = $request->suser ?? '';
...
if ($tuser != NULL) {
    ...
    $rootUser = DB::table('round1')->orderBy('id', 'asc')->first();

    if (!$rootUser) {
        return response()->json(['error' => 'No data found in round1 table']);
    }

    $username = $rootUser->user_id_fk;
}

$check = DB::table('round1')->where('user_id_fk', $username)->count();

if ($check == 0) {
    $username = DB::table('round1')->orderBy('id', 'asc')->value('user_id_fk');
}
...
// dd($username);
    ...
    // Main user data
    $user_data = DB::table('round1')->where('user_id_fk', $username)->first();
    $user_id = $user_data->id ?? null;
    $mydata = $user_data;

    if ($user_id != "") {
        $childs_1 = $this->find_users_db($user_id, 'Left');
        $childs_2 = $this->find_users_db($user_id, 'Right');

        $childs_3 = !empty($childs_1) ? $this->find_users_db($childs_1->id, 'Left') : [];
        $childs_4 = !empty($childs_1) ? $this->find_users_db($childs_1->id, 'Right') : [];

        $childs_5 = !empty($childs_2) ? $this->find_users_db($childs_2->id, 'Left') : [];
        $childs_6 = !empty($childs_2) ? $this->find_users_db($childs_2->id, 'Right') : [];
    } else {
        $childs_1 = $childs_2 = $childs_3 = $childs_4 = $childs_5 = $childs_6 = [];
    }
...
public function find_users_db($snode, $pos)
{
    $user_just_downline = DB::table('round1')
        ->where('Parentid', $snode)
        ->where('position', $pos)
        ->first();
    ...
    public function find_users2($snode,$pos) {
      // $this->load->model('Dashboard_model');
    $user_just_downline = Round1::where('Parentid',$snode)->where('position',$pos)->first();
         return $user_just_downline;
  }
